	...
	public static void adminMainMenu() {
		...
		case 7:
			...
			List<Patient> patients=dao.viewPatient();
			 ...
			 List<Patient> patientbillrange =patients.stream().filter(e->e.getFee()>=fees).collect(Collectors.toList());
			 if(patientbillrange.isEmpty()){
				 System.err.println("No Patient Found for the given Bill range");
			 }else{
			 for(Patient p: patientbillrange)
